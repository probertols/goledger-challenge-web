import type { AssetType, EntityDefinition } from '../../domain/entities/catalog';
...
function EntitySidebar({ entities, activeEntity, onSelect, stats }: EntitySidebarProps) {
  return (
    <div className="flex flex-col rounded-[28px] border border-[var(--color-paper)]/12 bg-[linear-gradient(180deg,rgba(24,33,37,0.92),rgba(97,112,125,0.35))] p-5 shadow-[var(--shadow-panel)] backdrop-blur-xl">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="mt-1 h-3 w-3 rounded-full" style={{ backgroundColor: entity.accent }} />
              </div>
            </button>
          );
        })}
      ...
      <div className="mt-auto grid grid-cols-2 gap-3 pt-5">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="rounded-2xl border border-[var(--color-paper)]/10 bg-[linear-gradient(180deg,rgba(232,93,117,0.08),rgba(97,112,125,0.12))] p-4"
          >
